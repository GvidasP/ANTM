...
def draw_cluster(cluster_labels, umap, name, show_2d_plot, path):
    ...
    if not os.path.exists(path + "/results/partioned_clusters"):
        os.mkdir(path + "/results/partioned_clusters")
...
def clustered_cent_df(clustered_df):
    ...
    for i in clustered_df:
        ...
        de["embedding_mean"] = de["embedding"].apply(
...
def dt_creator(clustered_df_cent):
    ...
    for i in range(len(clustered_df_cent)):
        t = (
            ...
            .copy()
            .reset_index()
            .rename(columns={"index": "cluster_num"})
        )
...
def alignment_procedure(
    dt, concat_cent, umap_n_neighbor=2, umap_n_components=5, min_cluster_size=2
):
    umap_args = {
        "n_neighbors": umap_n_neighbor,
...
def plot_alignment(
    df_tm, umap_embeddings_visualization, clusters_labels, path, show_3d_plot
):
    tm = df_tm[["window_num", "cluster_num", "C"]]
    tm_copy = tm.copy()
    tm_copy.loc[:, "name"] = tm.apply(
        lambda row: str(row["window_num"]) + "-" + str(row["cluster_num"]),
        axis=1,
    )
    tm = tm_copy
    tm = tm[tm["C"] != -1]
