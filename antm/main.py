...
from antm.aligned_clustering_layer import (
    aligned_umap,
    hdbscan_cluster,
    draw_cluster,
    clustered_df,
...
from antm.diversity_metrics import (
...
class ANTM:
    def __init__(
        ...
        umap_embeddings_clustering=None,
        umap_embeddings_visulization=None,
        device=None,
        partioned_clusttering_size=10,
        num_words=10,
        show_2d_plot=False,
        show_3d_plot=False,
        path=os.getcwd(),
    ...
    def fit(self, save=True):
        ...
        self.slices, self.arg1_umap, self.arg2_umap = sws(
            self.df_embedded, self.overlap, self.window_length
        )

        # Aligned dimensionality reduction
        if (
            self.umap_embeddings_clustering is None
            or self.umap_embeddings_visulization is None
        ):
            print("Aligned Dimension Reduction is initialized...")
            (
                self.umap_embeddings_clustering,
        ...
        self.clusters = hdbscan_cluster(
            self.umap_embeddings_clustering, self.partioned_clusttering_size
        )
        ...
        for i in range(len(self.clusters)):
            draw_cluster(
                self.clusters[i],
                self.umap_embeddings_visulization[i],
                "time_frame_" + str(i),
                show_2d_plot=self.show_2d_plot,
                path=self.path,
            )
        self.cluster_df = clustered_df(self.slices, self.clusters)
        self.clustered_df_cent, self.clustered_np_cent = clustered_cent_df(
            self.cluster_df
        )
    ...
    def save(self):
        ...
        with open(
            self.path + "/model/umap_embeddings_clustering", "wb"
        ) as fp:  # Pickling
            pickle.dump(self.umap_embeddings_clustering, fp)

        with open(
            self.path + "/model/umap_embeddings_visulization", "wb"
        ) as fp:  # Pickling
    ...
    def load(self):
        ...
        with open(
            self.path + "/model/umap_embeddings_visulization", "rb"
        ) as fp:
        ...
        self.evolving_topics = pd.read_pickle(
            self.path + "/model/evolving_topics"
        )
    ...
    def save_evolution_topics_plots(self, display=False):
        for j in range(len(self.list_tm)):
            ...
            for i in range(len(random_element)):
                ...
                plt.scatter(data[0], data[1], label=[win, list_words[i]])
                plt.legend()
            if not os.path.exists(self.path + "/results/evolving_topics"):
                os.mkdir(self.path + "/results/evolving_topics")
            plt.savefig(
                self.path
                + "/results/evolving_topics/topic_evolution_"
                + str(j)
                + ".png"
            )
            if display:
    ...
    def plot_clusters_over_time(self):
        ...
        plt.suptitle(
            "Dynamic Document Embeddings and Clusters in each Time Frame"
        )
    ...
    def get_periodwise_puw_diversity(self):
        self.periodwise_puw_diversity = [
            proportion_unique_words(period, topk=self.num_words)
            for period in self.topics
        ]
        return self.periodwise_puw_diversity

    def get_periodwise_pairwise_jaccard_diversity(self):
        self.periodwise_pairwise_jaccard_diversity = [
            pairwise_jaccard_diversity(period, topk=self.num_words)
            for period in self.topics
        ]
        return self.periodwise_pairwise_jaccard_diversity

    def get_periodwise_topic_coherence(self, model="c_npmi"):
        self.periodwise_topic_coherence = [
            coherence_model(
                period, self.tokens, self.dictionary, self.num_words, c_m=model
            )
            for period in self.topics
        ]
