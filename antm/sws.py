...
def sws(df_embedded, overlap, window_length):
    df_triple = [
        list(x)
        for x in zip(
            df_embedded.time.tolist(),
            df_embedded.index.tolist(),
            df_embedded.embedding.tolist(),
        )
    ]
    slices = slice_df(
        df_embedded,
        df_triple[0][0],
        df_triple[len(df_triple) - 1][0],
        window_length,
        overlap,
    )
    sliced_list_doc_vect, sliced_list_id = slice_by_year(
        df_triple,
        df_triple[0][0],
        df_triple[len(df_triple) - 1][0],
        window_length,
