...
def document_tokenize(cleaned_documents):
    ...
    document_tokens = [
        word_tokenize(document) for document in cleaned_documents
    ]
    tokens = [
        [token for token in document_tokens if token not in stop_words]
...
def doucments_lemmatizer(documents_tokens):
    tokens = [
        [WordNetLemmatizer().lemmatize(token) for token in doc_tokens]
...
def token_frequency_filter(documents_tokens, threshold):
    ...
    for doc_tokens in documents_tokens:
        for token in doc_tokens:
            frequency[token] += 1
    tokens = [
        [token for token in doc_tokens if frequency[token] > threshold]
        for doc_tokens in documents_tokens
    ]
    return tokens
